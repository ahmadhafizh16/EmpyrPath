		...
		async function checkAuth() {
			...
			try {
				...
				if (res.ok) {
					...
					setOidcLoginLabel(
						data.oidcLoginLabel || "Sign in with OIDC",
					);
				} else {
			...
			<aside className="relative flex flex-col justify-between overflow-hidden px-8 py-10 text-white md:w-1/2 md:px-14 md:py-14">
				...
				<div className="relative z-10 my-12">
					...
					<p className="mt-5 max-w-md text-base leading-relaxed text-white/85">
						Self-hosted AI router. 40+ providers, auto-fallback
						tiers, and the RTK token saver — never stop coding.
					</p>
				</div>
            ...
            aria-hidden so SR users don't hear the duplicated names. */}
				...
				>
					...
					<div className="flex w-max animate-marquee gap-2.5 pr-2.5">
						{[...MARQUEE_PROVIDERS, ...MARQUEE_PROVIDERS].map(
							(p, i) => (
								<span
									key={`${p.name}-${i}`}
							...
							>
								{!isRegister &&
									((authMode === "oidc" && !oidcConfigured) ||
										...
										<p className="rounded-xl bg-mm-surface px-3 py-2 text-xs leading-relaxed text-slate">
											OIDC is enabled but not yet
											configured. Password login still
											works for recovery.
										</p>
									)}
								...
								{!isRegister &&
									...
									oidcConfigured && (
										<p className="text-center text-xs text-steel">
											Password and OIDC login are both
											enabled.
										</p>
									)}
								...
								{!isRegister && hasPassword === false && (
									<p className="text-center text-xs text-stone">
										No custom password is set yet. The
										default works until you change it.
									</p>
								)}
							</form>
						) : (

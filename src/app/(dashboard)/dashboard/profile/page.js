...
export default function ProfilePage() {
		...
		try {
			...
			if (res.ok) {
				...
				setOidcForm({
					...
					oidcScopes:
						data?.oidcScopes || scopes || "openid profile email",
					oidcLoginLabel:
						data?.oidcLoginLabel ||
						loginLabel ||
						"Sign in with OIDC",
	...
	const testOidcConnection = async () => {
		...
		try {
			const saveRes = await fetch("/api/settings", {
				...
				body: JSON.stringify({
					authMode:
						oidcForm.authMode || settings.authMode || "password",
					oidcIssuerUrl: issuerUrl,
					oidcClientId: clientId,
					oidcScopes: scopes || "openid profile email",
					oidcLoginLabel:
						oidcForm.oidcLoginLabel.trim() || "Sign in with OIDC",
			...
			if (!saveRes.ok) {
				setOidcTestStatus({
					...
					message:
						saved.error ||
						"Failed to save OIDC settings before testing",
				});
				return;
			}
			...
			const res = await fetch("/api/auth/oidc/test", {
				...
				body: JSON.stringify({
					...
					scopes:
						saved.oidcScopes || scopes || "openid profile email",
				}),
			});
	...
	return (
		<div
			data-section={SECTIONS.profile.color}
		...
		>
						...
						<div className="flex items-start sm:items-center justify-between gap-4">
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs sm:text-sm text-text-muted">
									When ON, dashboard requires password. When
									OFF, access without login.
								</p>
							</div>
							<Toggle
				...
				<Card>
					...
					{oidcExpanded && (
						<div className="flex flex-col gap-4 mt-4">
							<p className="text-xs sm:text-sm text-text-muted">
								Use Authentik or any OIDC provider to sign in to
								the dashboard. You can enable password-only,
								OIDC-only, or both for the dashboard; model API
								access still uses API keys.
							</p>
							...
							<div className="flex flex-col gap-2">
								...
								<div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
									{[
										...
										const active =
											oidcForm.authMode === option.value;
										return (
											<button
												key={option.value}
												type="button"
							...
							{settings.authMode === "oidc" && (
								<p className="text-xs sm:text-sm text-amber-600 dark:text-amber-400">
									OIDC login is currently active. Password
									login is disabled until you switch back.
								</p>
							)}
					...
					<div className="flex flex-col gap-4">
						...
						<div className="flex items-start sm:items-center justify-between gap-4 pt-4 border-t border-border/50">
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs sm:text-sm text-text-muted">
									Cycle through providers in combos instead of
									always starting with first
								</p>
							</div>
							<Toggle
								checked={
					...
					<div className="flex flex-col gap-4">
						<div className="flex items-start sm:items-center justify-between gap-4">
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs sm:text-sm text-text-muted">
									Enable proxy for OAuth + provider outbound
									requests.
								</p>
							</div>
							<Toggle
						...
						{settings.outboundProxyEnabled === true && (
							...
							>
								<div className="flex flex-col gap-2">
									...
									/>
									<p className="text-xs sm:text-sm text-text-muted">
										Leave empty to inherit existing env
										proxy (if any).
									</p>
								</div>
				...
				<Card>
					...
					<div className="flex items-start sm:items-center justify-between gap-4">
						<div className="flex-1 min-w-0">
							...
							<p className="text-xs sm:text-sm text-text-muted">
								Record request details for inspection in the
								logs view
							</p>
						</div>
						<Toggle
			...
			>
				<p className="text-text-muted mb-3 text-sm">
					...
					{dbAuth.mode === "export" ? "export" : "import"} the
					database.
				</p>
				<Input
					type="password"
					...
					onKeyDown={(e) => {
						if (e.key === "Enter" && dbAuth.password)
							handleDbAuthConfirm();
					}}

...
export default function ProviderDetailPage() {
	...
	const isOAuth =
		!!OAUTH_PROVIDERS[providerId] || !!FREE_PROVIDERS[providerId];
	...
	useEffect(() => {
		...
		setSelectedModelIds((prev) => {
			if (
				...
				prev.every(
					(modelId, index) => modelId === nextSelectedModelIds[index],
				)
			) {
				return prev;
			}
				...
				{visibleModels.length === 0 ? (
					...
					<div className="flex flex-wrap gap-3">
						{visibleModels.map((model) => {
							...
							const existingAlias = Object.entries(
								modelAliases,
							).find(
								([, m]) =>
									m === fullModel || m === oldFormatModel,
							)?.[0];
							return (
								<ModelRow
									key={model.id}
									...
									onDeleteAlias={() =>
										handleDeleteAlias(existingAlias)
									}
								/>
							);
						})}
			...
			{isCompatible && providerNode && (
				<Card>
					...
					{connections.length > 0 && (
						<p className="text-sm text-text-muted">
							Only one connection is allowed per compatible node.
							Add another node if you need more connections.
						</p>
					)}
				</Card>
			)}
...
function PassthroughModelsSection({
	...
	return (
		<div className="flex flex-col gap-4">
			<p className="text-sm text-text-muted">
				OpenRouter supports any model. Add models and create aliases for
				quick access.
			</p>
...
function CompatibleModelsSection({
	...
	return (
		<div className="flex flex-col gap-4">
			<p className="text-sm text-text-muted">
				Add {isAnthropic ? "Anthropic" : "OpenAI"}-compatible models
				manually or import them from the /models endpoint.
			</p>
			...
			<div className="flex items-end gap-2 flex-wrap">
				<div className="flex-1 min-w-[240px]">
					...
					<input
						...
						onKeyDown={(e) => e.key === "Enter" && handleAdd()}
						placeholder={
							isAnthropic ? "claude-3-opus-20240229" : "gpt-4o"
						}
						className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary"
					/>
				</div>
						...
						<div className="flex gap-2">
							...
							<div className="pt-6">
								<Button
									onClick={handleValidate}
									disabled={
										!formData.apiKey || validating || saving
									}
									variant="secondary"
								>
									{validating ? "Checking..." : "Check"}
